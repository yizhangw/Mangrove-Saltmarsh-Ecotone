...
if year==1 && ets==1 && Restart==0
    disp(['Vegetation will be assigned to the corresponding cells at the Year ' num2str(year) ' with a Total Num. of ' num2str(num0) ' each!' ]);
...
elseif ets==1
    trv_trd_dh(year-1,t_eco_year) = {trv_trd}; % save trv, trd, I and C based on last ets
else
    trv_trd_dh(year,ets-1)        = {trv_trd}; % save trv, trd, I and C based on last ets
end
...
elseif ets==1 && ~isempty(trv_trd) % 09/10/2018 exclude null trv_trd
    ...
    trv_trd_temp              = trv_trd_dh{year-1,end};
