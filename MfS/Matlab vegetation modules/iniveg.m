...
eco_timestep = (t_days_year*24*60)/t_eco_year; %the amount of the eco. minutes/ets
% t_days_year is a parameter preset in general_input code as 366
...
if VegPres > 0 % check if there is dynamic vegetation presentm (1 =  vegetation present, 0 = no vegetation present)
    ...
    % Intitialize matrices for saving parameters
    ...
    G1                   = zeros(num_veg_types,1); % Growth constant 1, G (cm/year)
    ...
    for nv = 1:num_veg_types % start loop over vegetation types
        ...
        clear FID data mat_veg life_veg_temp nls
        ...
        G1(nv)              = life_veg_char{nv,1}(1,3); % Growth constant 1, G (cm/year)
        ...
        Shoot_height0(nv)   = (-10+b2(nv)*stem_d0(nv)-b3(nv)*stem_d0(nv)^2)/100; % shoot height, (m).
