...
%--------------------To grow, or not to grow, this is a question---------------------%
...
elseif ~isempty(trv_trd) % 09/10/2018 exclude null trv_trd
    if ets==1 || ets==2 || ets==3 || ets==4 || ets==5 || ets==6 || ets==7
        ...
        for nv=1:num_veg_types
            vegtype  = find(trv_trd(:,11)==nv); % vegetation type
            % Partial diameter
            ...
            D_D_T    = G1(nv)*D_prev.*(1-D_prev.*H_prev/(Dmax(nv)*Hmax(nv))).*IC_prev./((274+3*b2(nv)*D_prev-4*b3(nv)*D_prev.*D_prev)); % diameter increasement (cm) per ets
            D_cur    = D_prev + D_D_T;
            D_cur(D_cur > Dmax(nv)) = Dmax(nv);
            H_cur    = -10+b2(nv)*D_cur-b3(nv)*D_cur.^2;
        ...
        for nv=1:num_veg_types
            ...
            H_prev   = trv_trd(vegtype, 7)*100; % Pre-ets shoot height, H_prev(cm)
            IC_prev  = trv_trd(vegtype,18); % Pre-ets I*C stress
            D_D_T    = G2(nv)*D_prev.*(1-D_prev.*H_prev/(Dmax(nv)*Hmax(nv))).*IC_prev./((274+3*b2(nv)*D_prev-4*b3(nv)*D_prev.*D_prev)); % diameter increasement (cm) per ets
            D_cur    = D_prev + D_D_T;
            H_cur    = -10+b2(nv)*D_cur-b3(nv)*D_cur.^2;
            trv_trd(vegtype,13) = D_cur; % update the new diameter, shoot_diameter(cm)
            trv_trd(vegtype, 7) = H_cur/100; % update the new shoot height, TRD_height (m)
...
clear trv_trd_addIC vegtype D_prev H_prev IC_prev D_D_T D_cur H_cur